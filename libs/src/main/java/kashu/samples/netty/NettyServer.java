...
public class NettyServer {
	...
	public void run() throws Exception {
		 ...
		 * NioEventLoopGroup will be used. The first one, often called 'boss', accepts an incoming
		 * connection. The second one, often called 'worker', handles the traffic of the accepted
		 * connection once the boss accepts the connection and registers the accepted connection to
		 * the worker. How many Threads are used and how they are mapped to the created Channels
		...
		try {
			/**
			 * 2. ServerBootstrap is a helper class that sets up a server. You can set up the server using
			 * a Channel directly. However, please note that this is a tedious process, and you do not
			 * need to do that in most cases.
			 *
			 * 3. Here, we specify to use the NioServerSocketChannel class which is used to instantiate a
			 ...
			 * new Channel by adding some handlers such as DiscardServerHandler to implement your network
			 * application. As the application gets complicated, it is likely that you will add more
			...
			b.group(bossGroup, workerGroup)
				 ...
				 *
				 * 6. Did you notice option() and childOption()? option() is for the NioServerSocketChannel
	...
	public static void main(String[] args) throws Exception {
		...
		new NettyServer(port, new TimeServerHandler()).run();
	}
}
